...
public class CategoryListScreen {
	...
	public static final String RECORD_CATEGORIES_XPATH	= "//tbody/tr";
	...
	public static void deleteCategory(WebDriver driver, String index, boolean isOK) throws Exception {
		int recordBefore = Utilities.getXpathCount(driver, RECORD_CATEGORIES_XPATH);
		Utilities.click(driver, By.xpath(DELETE_BTN_XPATH.replace(Constant.INDEX_TEXT, index)));
		if (isOK) {
			driver.switchTo().alert().accept();
			Thread.sleep(3000);
			int recordAfter = Utilities.getXpathCount(driver, RECORD_CATEGORIES_XPATH);
